...
torch.float8_e4m3fnuz = dtype("float8_e4m3fnuz")
torch.float8_e5m2fnuz = dtype("float8_e5m2fnuz")
...
def add_stages(self, stages, options):
    ...
    real_ttir, ttadapter, npubin = stages.values()
    def ttir(src, metadata):
        print(src)
        return real_ttir(src, metadata)
    stages.clear()
    stages["ttir"]      = ttir
